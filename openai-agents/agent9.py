...
1. Using large language models (LLMs) with simple prompting techniques, such as asking for "Steps for XYZ" or "What are the subgoals for achieving XYZ?"
...
Additionally, an approach known as LLM+P, as described by Liu et al. (2023), utilizes an external classical planner for long-horizon planning. This method involves translating the task into a Planning Domain Definition Language (PDDL) format, generating a plan using a classical planner, and then translating that plan back into natural language.

Common extensions of the standard method include:

1. **Chain of Thought (CoT)**: This technique enhances performance on complex tasks by instructing the model to "think step by step," breaking down larger tasks into smaller, more manageable ones. This method also provides insights into the model's reasoning process.

2. **Tree of Thoughts**: An extension of CoT, this method explores multiple reasoning possibilities at each step. It decomposes the problem into various thought steps and generates multiple thoughts for each step, structuring these thoughts in a tree format, with evaluation based on either breadth-first search (BFS) or depth-first search (DFS).
